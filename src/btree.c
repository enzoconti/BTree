...
void insercao_btree(FILE*fp, reg_cabecalho_arvore*h, int key, int data_rrn_4insertion){
    ...
    if(flag_retorno == PROMOTION){ // houve split na raiz ou a arvore estava vazia
        ...
        if(h->noRaiz != -1){
        new_root->folha[0] = '0';
        } // em caso contrario o no eh folha nao o alteramos ja que o padrao de cria_registro_dados_indice() eh inicializar com folha[0] = 1
    ...
    h->nroChavesTotal++; // unica atualizacao feita independentemente de ter havido split na raiz
